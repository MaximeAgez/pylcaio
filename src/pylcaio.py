...
class Hybridize_ecoinvent:
    def __init__(self, bw2_project_name, ecoinvent_db_name, ecoinvent_version, path_to_exiobase):
        ...
        # Namibia 2-letter code (NA) is interpreted as NaN by pandas after reading from Excel
        self.filter['Hybridized processes'].location = self.filter['Hybridized processes'].location.fillna('NA')
        ...
        self.filter['Empty and aggregated processes'].location = self.filter[
            'Empty and aggregated processes'].location.fillna('NA')
    ...
    def connect_filter_to_codes(self):
        """
        This function takes the uuid codes of the user-generated ecoinvent database and connects those codes to all the
        necessary information contained in the filter attribute.
        """

        self.logger.info("Connecting user-defined ecoinvent codes to information matrices...")
...
class Hybridize_regioinvent:
    def __init__(self, bw2_project_name, regioinvent_db_name, ecoinvent_db_name, ecoinvent_version, path_to_exiobase):
        ...
        # Namibia 2-letter code (NA) is interpreted as NaN by pandas after reading from Excel
        self.filter_ei['Hybridized processes'].location = self.filter_ei['Hybridized processes'].location.fillna('NA')
        self.filter_ei['Market processes'].location = self.filter_ei['Market processes'].location.fillna('NA')
        self.filter_ei['Internal and activities'].location = self.filter_ei['Internal and activities'].location.fillna('NA')
        self.filter_ei['Empty and aggregated processes'].location = self.filter_ei[
            'Empty and aggregated processes'].location.fillna('NA')
    ...
    def connect_filter_to_codes(self):
        ...
        self.logger.info("Connecting user-defined ecoinvent and regioinvent codes to information matrices...")
